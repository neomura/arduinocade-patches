...
# Patch pad input.
sed -i 's/^#define ATARI_FLASHBACK/\/\/#define ATARI_FLASHBACK/g' ./build/games/$1/config.h
sed -i 's/^#include "config.h"$/#include "config.h"\nvoid padSetup();/g' ./build/games/$1/arduinocade.cpp
sed -i 's/^    video_start();$/    padSetup();\n    video_start();/g' ./build/games/$1/arduinocade.cpp
sed -i 's/^#include "video.h"$/#include "video.h"\nvoid padPoll();/g' ./build/games/$1/video.cpp
sed -i 's/^    _line_count++;$/    if (v_state == STATE_PRE)\n    {\n        padPoll();\n    }\n    _line_count++;/g' ./build/games/$1/video.cpp
